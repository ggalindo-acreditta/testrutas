...
function validateDate(value, required, onlyRequired) {
  ...
  var validatedPatternList = ['date-time', 'date-time2', 'date-time3', 'date-time4', 'date-time5',
    'date-time6', 'date-time7', 'date-time8', 'date'];
  var validado;
  var formatPattern;
  var i;
...
function validateSchema(schemaIn, schemaTpl, options) {
  ...
  if (_.isEmpty(schemaTpl) && !_.isEmpty(schemaIn)) {
    requiredErrList.push(options.errorFormatter(apiModelError.AM010.code, '', apiModelError.AM010.message, ''));
  }
  ...
  for (var param in schemaTpl) {
    ...
    if (schemaTpl[param].hasOwnProperty('type')) {
      if (dataTypes.indexOf(schemaTpl[param].type) >= 0) {
        ...
        requiredErrList.push(options.errorFormatter(apiModelError.AM008.code, param, apiModelError.AM008.message,
    ...
    */
    ...
    if (_.has(schemaIn, param)) {
      ...
      if (!validateRequired(currentValue, currentType, currentRequired)) {
        ...
        requiredErrList.push(options.errorFormatter(apiModelError.AM001.code, param, apiModelError.AM001.message,
          currentValue));
        ...
        validateErrList.push(options.errorFormatter(errorCode.code, param, errorCode.message, currentValue));
      }
    } else {
      // No se consigue, entonces incluirlo en la lista
      if (currentRequired) {
        requiredErrList.push(options.errorFormatter(apiModelError.AM001.code, param, apiModelError.AM001.code, ''));
      }
    }
  }
    ...
    getError: function(errorList) {
      ...
      if (errorList.length > 0) {
        ...
        for (i = 0; i < errorList.length; i++) {
          messageList.push([errorList[i].param, ': ', errorList[i].msg].join(''));
        }
