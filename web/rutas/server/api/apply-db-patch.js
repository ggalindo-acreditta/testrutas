...
var cwd = __dirname;
var patchListDir = ''; // Directorio donde se encuentran los parches

// Por defecto busca primero en el directorio local ./db-patch/
// Si no se encuentra alli entonces busca en ../db/dev/upgrade/
// de esta forma se cubre si esta en ambiente de desarrollo o en produccion
if (fs.existsSync(path.join(cwd, 'db-patch', 'patch-list.js'))) {
  patchList = require(path.join(cwd, 'db-patch', 'patch-list')).patches;
  patchListDir = path.join(cwd, 'db-patch');
} else if (fs.existsSync(path.join(cwd, '..', 'db', 'dev', 'upgrade', 'patch-list.js'))) {
  patchList = require(path.join(cwd, '..', 'db', 'dev', 'upgrade', 'patch-list')).patches;
  patchListDir = path.join(cwd, '..', 'db', 'dev', 'upgrade');
} else {
  console.error('No se tiene acceso la lista de parches');
  process.exit(1);
}
...
var applyPatch = function(cb) {
  var patch = currPatch;
  var patchFile = path.join(patchListDir, patch.patch);
  var sql = '';
