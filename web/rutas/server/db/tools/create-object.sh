...
l_create_sequence() {
  ...
  if [ ! -d "$3/$lobjeto" ]; then
    mkdir -p "$3/$lobjeto"
    if [[ $? != 0 ]]; then
        log "Command failed: mkdir -p $3/$lobjeto"
        exit 1
    fi
  fi

  declare -a filearr=("auth" "def")
  for lext in "${filearr[@]}"
  do
...
# Para el cast no se necesita el nombre del esquema
  case $1 in
    cast|schema|role|sequence|user)
      if [ -z "$1" ]; then
        l_help
        exit 1
      fi
...
l_help() {
  ...
  log "Uso: $binfilename cast|domain|function|schema|table|type|view|role|sequence|user schema-name object-name output-dir"
  ...
  log "  para crear un role: $binfilename role role-name output-dir"
  log "  para crear un user: $binfilename user user-name output-dir"
}
